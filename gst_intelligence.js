/**
 * gst_intelligence.js
 * GST Lens — Smart Intelligence Engine v2.0
 * Rule-based, 100% offline, zero API calls
 *
 * FIXES in v2.0:
 *  1. detectGSTRate() — now also reads explicit rate text like "CGST 9%" / "IGST @18%"
 *     so 0% is never shown when a real rate exists in the OCR text.
 *  2. checkITCEligibility() — now receives gstRate; returns blocked when rate=0.
 *  3. analyse() — category now always uses detectCategory() first, falling back to
 *     data.category only when detectCategory returns the generic fallback.
 *  4. analyse() — passes gstRate into checkITCEligibility so the fix above fires.
 *  5. detectGSTRate() — now accepts optional rawOcrText for rate-text fallback.
 */
...
const GSTIntelligence = (() => {
  ...
  const ITC_BLOCKED = [
    { cat: 'Food & Dining',       reason: 'Food/beverages — ITC blocked under Section 17(5)(b)' },
    { cat: 'Healthcare',          reason: 'Healthcare — ITC blocked unless it is your core business' },
    { cat: 'Education',           reason: 'Education — generally exempt supply, ITC not applicable' },
    { cat: 'Fuel & Energy',       reason: 'Motor fuel — ITC blocked under Section 17(5)(a)' },
    { cat: 'Food & Bev',          reason: 'Food/beverages — ITC blocked under Section 17(5)(b)' },
    { cat: 'Medical',             reason: 'Medical supply — ITC blocked unless core business' },
  ];
  ...
  function detectCategory(vendorName, hsnCode, ocrCategory) {
    ...
    if (hsnCode) {
      ...
      if (hsnEntry) {
        ...
        return { cat: hsnEntry, suggestedRate: 18, source: 'hsn' };
      }
    }

    // 3. Fall back to OCR-supplied category (better than nothing)
  ...
  // ── FIX 1: Detect GST rate — computation + explicit text fallback ─────
  /**
   * @param {Object} data - parsed invoice fields
   * @param {string} [rawOcrText] - original OCR text to extract explicit rate mentions
  ...
  function detectGSTRate(data, rawOcrText) {
    ...
    // ── Path A: compute from amounts (primary) ──────────────────────────
    ...
    // ── Path B: read explicit rate text from OCR (FIX for 0% false-positive)
    // Handles: "CGST 9%", "CGST @ 9%", "IGST @18%", "GST Rate: 12%", "C6ST 9%", "S6ST 9%"
    if (rawOcrText) {
      const txt = rawOcrText
        .replace(/C6ST/gi, 'CGST')
        .replace(/S6ST/gi, 'SGST')
        .replace(/I6ST/gi, 'IGST');

      // Try IGST rate first (interstate)
      const igstRateMatch = txt.match(/I[Gg]ST\s*[@%\s]*(\d+(?:\.\d+)?)\s*%/i);
      if (igstRateMatch) {
        const r = parseFloat(igstRateMatch[1]);
        if (r > 0) return VALID.reduce((a, b) => Math.abs(b - r) < Math.abs(a - r) ? b : a);
      }

      // Try CGST rate (intrastate — multiply by 2 to get total GST)
      const cgstRateMatch = txt.match(/C[Gg]ST\s*[@%\s]*(\d+(?:\.\d+)?)\s*%/i);
      if (cgstRateMatch) {
        const half = parseFloat(cgstRateMatch[1]);
        if (half > 0) {
          const total = half * 2;
          return VALID.reduce((a, b) => Math.abs(b - total) < Math.abs(a - total) ? b : a);
        }
      }

      // Try generic "GST Rate: X%" or "Tax Rate: X%"
      const genericRateMatch = txt.match(/(?:GST|Tax)\s*[Rr]ate\s*[:\-@]?\s*(\d+(?:\.\d+)?)\s*%/i);
      if (genericRateMatch) {
        const r = parseFloat(genericRateMatch[1]);
        if (r > 0) return VALID.reduce((a, b) => Math.abs(b - r) < Math.abs(a - r) ? b : a);
      }

      // Try standalone "18%" or "12%" when near a tax label on same line
      const taxLineMatch = txt.match(/(?:cgst|sgst|igst|gst|tax)[^\n]{0,30}?(\d+(?:\.\d+)?)\s*%/gi);
      if (taxLineMatch) {
        for (const line of taxLineMatch) {
          const m = line.match(/(\d+(?:\.\d+)?)\s*%/);
          if (m) {
            const r = parseFloat(m[1]);
            // CGST/SGST lines have half-rates; detect by label
            const isHalf = /cgst|sgst/i.test(line);
            const effective = isHalf ? r * 2 : r;
            if (effective > 0 && effective <= 28) {
              return VALID.reduce((a, b) => Math.abs(b - effective) < Math.abs(a - effective) ? b : a);
            }
          }
        }
      }
    ...
    // ── Path C: HSN-based rate (last resort before 0) ──────────────────
    if (data.hsnCode) {
      const h = String(data.hsnCode).substring(0, 4);
      for (const rm of HSN_RATE_MAP) {
        if (rm.prefix.some(p => h.startsWith(p.substring(0, 4)))) return rm.rate;
      }
    }
  ...
  // ── FIX 2: ITC eligibility now checks gstRate ─────────────────────────
  /**
   * @param {string} category
   * @param {number} totalAmount
   * @param {boolean} hasGSTIN
   * @param {number} gstRate  ← NEW parameter
   * @returns {{ eligible: boolean, reason: string, flag?: string }}
   */
  function checkITCEligibility(category, totalAmount, hasGSTIN, gstRate) {
    ...
    // FIX: zero-rated/exempt supply — no tax = no ITC regardless of GSTIN
    if (gstRate === 0) {
      return { eligible: false, reason: 'Zero/exempt supply — no GST charged, so no ITC to claim' };
    }
  ...
  // ── Pick smart tip ────────────────────────────────────────────────────
  function pickTip(data, isInterstate, category, total, gstRate) {
    const pools = [];
  ...
  // ── FIX 3 + 4: MAIN analyse — correct category priority, passes gstRate ─
  /**
   * @param {Object} data   - parsed invoice (from parseGSTFields)
   * @param {string} [rawOcrText] - original OCR text (for rate extraction fallback)
   */
  function analyse(data, rawOcrText) {
    ...
    // FIX 1: pass rawOcrText so detectGSTRate can read explicit rate text
    const gstRate    = detectGSTRate(data, rawOcrText);
    const hsnDesc    = getHSNDescription(data.hsnCode);

    // FIX 3: detectCategory() wins; data.category (OCR guess) is only the last fallback
    const catResult  = detectCategory(data.vendorName, data.hsnCode, data.category);
    const category   = catResult.cat;

    // FIX 4: pass gstRate into ITC check
    const itcResult  = checkITCEligibility(category, total, !!(data.gstin), gstRate);
    const tags       = generateTags(data, isInterstate, gstRate, itcResult);
    const tip        = pickTip(data, isInterstate, category, total, gstRate);
  ...
  function renderCard(data, containerId, rawOcrText) {
    ...
    setTimeout(() => {
      // Pass rawOcrText so analyse() can extract explicit rates
