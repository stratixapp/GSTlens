...
(function () {
  ...
  let authState = { screen: 'login', studentId: '', error: null, captchaCode: Auth.generateCaptcha() };
  ...
  function doLogout() {
    ...
    Store.logout();
    authenticated = false;
    authState = { screen: 'login', studentId: '', error: null, captchaCode: Auth.generateCaptcha() };
    renderFull();
  }
  ...
  function submitAuthForm(isRegister) {
    ...
    if (isRegister) {
      ...
      const result = Auth.register(studentIdVal, passwordVal);
  ...
  root.addEventListener('keydown', function (e) {
    ...
    if (!['auth-studentId', 'auth-password', 'auth-confirmPassword', 'auth-captcha'].includes(e.target.id)) return;
    e.preventDefault();
  ...
  root.addEventListener('click', function (e) {
    ...
    if (action === 'approve-vendor') {
      const fields = readDrawerFields();
      Store.actions.approveVendor(btn.getAttribute('data-id'), fields);
      uiState.ephemeral.vendorApprovalDrawerOpenId = null;
      renderContentOnly();
      renderShellChrome();
      return;
    ...
    if (action === 'approve-po') {
      const fields = readDrawerFields();
      Store.actions.approvePO(btn.getAttribute('data-id'), { comment: fields.comment, signature: fields.signature });
      renderContentOnly();
      renderShellChrome();
      return;
    }
